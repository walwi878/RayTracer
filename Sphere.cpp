...
std::vector<RayIntersection> Sphere::intersect(const Ray& ray) const {
	...
	// Intersection is of form ad^2 + bd + c, where d is distance along ray
	...
	case 0:
		...
		t = -b/(2*a);
		if (t > 0) {
			// Intersection is in front of the ray's origin
	...
	case 1:
		...
		break;
	default:
		// If outside range [-1,1], just case
		std::cerr << "Sign(x) should be in range [-1, 1]" << std::endl;
		exit(-1);
		break;
	}
